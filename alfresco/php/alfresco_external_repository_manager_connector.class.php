...
namespace common\extensions\external_repository_manager\implementation\alfesco;

use common\libraries\TimeUtil;
use common\libraries\alfresco_fs_object;
use common\libraries\Path;
use common\libraries\Translation;
use common\libraries\Request;
use common\libraries\Session;
use common\libraries\alfresco_fs_store;
use common\libraries\alfresco_fs_mystuff;
use common\libraries\alfresco_fs_history;
use common\libraries\alfresco_fs_lastobjects;
use common\libraries\alfresco_fs_datastream;
use common\libraries\alfresco_fs_search;
use common\libraries\alfrescoProxy;
...
require_once Path::get_common_libraries_class_path() . '/alfresco/lib.php';
...
/**
 * Main object to connect Chamilo to the alfresco repository.
...
class AlfrescoExternalRepositoryManagerConnector extends ExternalRepositoryManagerConnector {
     ...
     *
     * To add a language modify the alfresco ressources and add the translation here.
    ...
    /**
     * Returns the external id for the current user. That is the one that is used to identify the current user on the alfresco server.
    ...
    public function get_default_store() {
        ...
        $result = new alfresco_fs_store(Translation::get_instance()->translate('root'));
        $result->add(new alfresco_fs_mystuff(self::DOCUMENTS_MY_STUFF, Translation::get('mystuff'), $owner));
        $result->add($history = new alfresco_fs_store(Translation::get_instance()->translate('history')));
        $history->add(new alfresco_fs_history(Translation::get('today'), TimeUtil::today(), null, $owner, self::DOCUMENTS_TODAY));
        $history->add(new alfresco_fs_history(Translation::get('this_week'), $this_week, null, $owner, self::DOCUMENTS_THIS_WEEK));
        $history->add(new alfresco_fs_history(Translation::get('last_week'), $last_week, $this_week, $owner, self::DOCUMENTS_LAST_WEEK));
        $history->add(new alfresco_fs_history(Translation::get('two_weeks_ago'), $two_weeks_ago, $last_week, $owner, self::DOCUMENTS_TWO_WEEKS_AGO));
        $history->add(new alfresco_fs_history(Translation::get('three_weeks_ago'), $three_weeks_ago, $two_weeks_ago, $owner, self::DOCUMENTS_THREE_WEEKS_AGO));

        $result->aggregate(new alfresco_fs_lastobjects('', false, $owner));

        $history->set_class('alfresco_history');
        return $result;
    }

    /**
     * Returns the RestConfig alfresco config entered by the plugin. Used by the alfresco proxy to connect to the alfresco server.
     *
     * @param $external_repository_id
     * @return RestConfig
     */
    public function get_alfresco_config($external_repository_id = false) {
    ...
    private $_alfresco = false;

    /**
     * Returns a alfresco proxy.
     *
     * @param $external_repository_id
     * @return alfrescoProxy
     */
    public function get_alfresco($external_repository_id=false) {
        if ($this->_alfresco) {
            return $this->_alfresco;
        }
        $this->_alfresco = new alfrescoProxy($this->get_alfresco_config($external_repository_id));
        return $this->_alfresco;
    }
    ...
    public function get_api_extender() {
        ...
        if ($api_name) {
            $path = dirname(__FILE__) . '/component/api/' . $api_name . '/alfresco_' . $api_name . '_connector_extender.class.php';
            require_once $path;

            $class = __NAMESPACE__ . '\alfresco' . ucfirst($api_name) . 'ExternalRepositoryConnectorExtender';
     ...
     * @return alfrescoExternalRepositoryObject|false
     *
     */
    function retrieve_external_repository_object($pid, $return_active_only = true) {
        $alfresco = $this->get_alfresco();
        try {
            $xml = $alfresco->get_object_xml($pid);
        } catch (Exception $e) {
        ...
        if ($result == false) {
            ...
            alfrescoExternalRepositoryManager::delete_synchronization_data($pid);
        }
        return $result;
    }
    ...
    function retrieve_datastreams($pid) {
        $alfresco = $this->get_alfresco();
        $fs = new alfresco_fs_object($pid, '', self::get_owner_id(), time(), time());
        $result = $fs->query($alfresco);
        return $result;
    }

    /**
     * Returns the url used to access the datasream's content directly from alfresco.
     * Used for external access. I.e. harvester. Can be changed in ConnectorExtender.
     * Direct access to alfresco should not be provided to the end-user as it would bypass internal authentication.
     *
     * @param string $pid
     * @param string $dsID
     */
    function get_datastream_content_url($pid, $dsID) {
        ...
        if ($url = ExternalSetting::get('ContentAccessUrl', $external_repository_id)) {
            ...
            $alfresco = $this->get_alfresco();
            return $alfresco->get_datastream_content_url($pid, $dsID);
        }
    }
    ...
    public function retrieve_datastream_content($pid, $dsID) {
        $alfresco = $this->get_alfresco();
        return $alfresco->get_datastream_content($pid, $dsID);
    }

    /**
     * Returns object's metadata from alfresco in an associative array.
     *
     * @param string $pid
     * @return array
     */
    public function retrieve_object_metadata($pid) {
        ...
        $alfresco = $this->get_alfresco();
        $profile = $alfresco->get_object_profile($pid);
        $result = array();
    ...
    public function get_object_by_label($label, $owner=true, $collection='') {
        ...
        $result = $alfresco->get_object_by_label($label, $owner, $collection);
        return $result;
    }
    ...
    public function ingest($xml_content, $pid=0, $label='', $owner=false) {
        ...
        $alfresco = $this->get_alfresco();
        return $alfresco->ingest($xml_content, $pid, $label, $owner);
    }
    ...
    function delete_external_repository_object($id) {
        $alfresco = $this->get_alfresco();
        try {
            $result = $alfresco->delete_object($id);
            alfrescoExternalRepositoryManager::delete_synchronization_data($id);
            return true;
        } catch (Exception $e) {
            alfrescoExternalRepositoryManager::delete_synchronization_data($id);
            return false;
    ...
    function purge_external_repository_object($id) {
        $alfresco = $this->get_alfresco();
        try {
            $result = $alfresco->purge_object($id);
            alfrescoExternalRepositoryManager::delete_synchronization_data($id);
            return true;
        } catch (Exception $e) {
            alfrescoExternalRepositoryManager::delete_synchronization_data($id);
    ...
    function count_external_repository_objects($condition) {
        $folder = Request::get(alfrescoExternalRepositoryManager::PARAM_FOLDER);
        $store = $this->get_store();

        if ($condition) {
            $owner = $this->get_owner_id();
            $search = new alfresco_fs_search('', $condition, alfresco_fs_search::SEARCH_LEVEL_FUZZI, false, false, $owner, $count, $offset, $order_property);
            $result = $search->count($this->get_alfresco());
            return $result;
        } else if ($folder) {
            $fs = $store->find($folder);
            $result = $fs->count($this->get_alfresco());
            if ($fs instanceof alfresco_fs_store) {
                $result -= count($fs->get_children());
            }
            return $result;
        } else {
            $result = $store->count($this->get_alfresco());
    ...
    function retrieve_external_repository_objects($condition, $order_property, $offset, $count) {
        ...
        if ($condition) {
            $owner = $this->get_owner_id();
            $search = new alfresco_fs_search('', $condition, alfresco_fs_search::SEARCH_LEVEL_FUZZI, false, false, $owner, $order_property, $limit, $offset);
            $alfresco = $this->get_alfresco();
            $items = $search->query($alfresco, $sort, $limit, $offset);

            foreach ($items as $item) {
                if ($item instanceof alfresco_fs_object) {
            ...
            $result = array();
            $alfresco = $this->get_alfresco();
            $items = $fs->query($alfresco, $sort, $limit, $offset);
            foreach ($items as $item) {
                if ($item instanceof alfresco_fs_object) {
                    $object = $this->fs_to_object($item);
                    $result[] = $object;
            ...
            $result = array();
            $alfresco = $this->get_alfresco();
            $items = $store->query($alfresco, $sort, $limit, $offset);
            foreach ($items as $item) {
                if ($item instanceof alfresco_fs_object) {
                    $object = $this->fs_to_object($item);
                    $result[] = $object;
    ...
    function update_thumbnail($pid, $name, $path, $mime_type) {
        ...
        $alfresco = $this->get_alfresco();
        return $alfresco->update_datastream($pid, 'THUMBNAIL', $name, $content, $mime_type, false);
    }
    ...
    function get_thumbnail_content($path) {
        ...
        if ($width == 0) {//Unable to deternime image size
            ...
            $tmp = Path::get_temp_path() . 'f' . Session::get_user_id() . md5(uniqid('alfresco_thumb'));
            $ratio = $size[1] / $size[0];
            $ratio = $ratio ? $ratio : 1;
    ...
    /**
     * Creates a alfrescoExternalRepositoryObject based on a alfresco_fs_object definintion;
     *
     * @param $fs
     * @return alfrescoExternalRepositoryObject
     */
    function fs_to_object(alfresco_fs_object $item) {
        $object = new alfrescoExternalRepositoryObject();
    ...
    public function determine_rights(alfresco_fs_object $item) {
        ...
        $rights = array();
        $rights[alfrescoExternalRepositoryObject::RIGHT_USE] = true;
        $rights[alfrescoExternalRepositoryObject::RIGHT_EDIT] = $can_edit;
        $rights[alfrescoExternalRepositoryObject::RIGHT_DELETE] = $can_edit;
        $rights[alfrescoExternalRepositoryObject::RIGHT_DOWNLOAD] = true;
        return $rights;
    }
    ...
    /**
     * Returns a alfrescoExternalRepositoryObject from the foxml description.
    ...
    protected function foxml_to_object($xml) {
        $result = new alfrescoExternalRepositoryObject();
        ...
        $config = $this->get_alfresco()->get_config();
        ...
        foreach ($dss as $ds) {
            ...
            $datastreams[$dsID] = new alfresco_fs_datastream($pid, $dsID, $title, $mime_type, $source);
